...
import wjdwo1104.hello.boot.spring5boot.service.PdsService;
...
public class PdsController {
    ...
    public String list(Model m, @PathVariable Integer cpg){
        ...
        m.addAttribute("cntpg",psrv.countPds());//총페이지
    ...
    @GetMapping("/down/{pno}")
    public ResponseEntity<Resource> down(Model m, @PathVariable String pno) {
        logger.info("pds/down 호출!!");

        //업로드한 파일에 대한 파일명 알아냄
        String fname = psrv.readOnePdsAttach(pno);
        //알아낸 파일명을 이용해서 header와 리소스 객체 생성
        Map<String, Object> objs = psrv.getHeaderResouce(fname);
